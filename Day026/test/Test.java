...
class PhoneBook {
    ...
    private Vector<PhoneInfo> infoVector;

    private PhoneBook(int sizePhoneInfo) {
        infoVector = new Vector<>(sizePhoneInfo);
    ...
    public int search(String name) {
        cntOfPhone = infoVector.size();
        for (int i = 0; i < cntOfPhone; i++) {
            if (infoVector.get(i).getName().compareTo(name) == 0)
                return i;
        }
        return -1;
    }
